...
describe('recursive', () => {
  ...
  test('when level set to 1 and only 1 level of directories will return them all', async () => {
    ...
    expect((await listSubdirectories({ path: testPath, levels: 1 })).length).toEqual(2)
  })
  ...
  test('when level set to 2, and only 2 levels of directories will return them all', async () => {
    ...
    expect((await listSubdirectories({ path: testPath, levels: 2 })).length).toEqual(4)
  })
  ...
  test('when level set to 3, and only 3 levels of directories will return them all', async () => {
    ...
    expect((await listSubdirectories({ path: testPath, levels: 3 })).length).toEqual(6)
  })
  ...
  test('when level set to 1, and 2 levels of directories will only return level 1', async () => {
    ...
    expect((await listSubdirectories({ path: testPath, levels: 1 })).length).toEqual(1)
  })
