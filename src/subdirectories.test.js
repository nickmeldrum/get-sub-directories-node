...
describe('subdirectories', () => {
  ...
  test('returns an array', async () => {
    expect(await listSubdirectories(testPath)).toEqual([])
  })

  test('returns a subdirectory if one exists', async () => {
    await fs.ensureDir(path.join(testPath, randomString()))
    expect((await listSubdirectories(testPath)).length).toEqual(1)
  })
  ...
  test('returns the full path to a subdirectory', async () => {
    ...
    expect((await listSubdirectories(testPath))[0]).toEqual(path.join(testPath, subDirectoryName))
  })
  ...
  test('returns many subdirectories if they exist', async () => {
    ...
    await fs.ensureDir(path.join(testPath, randomString()))
    await fs.ensureDir(path.join(testPath, randomString()))
    expect((await listSubdirectories(testPath)).length).toEqual(3)
  })
  ...
  test('throws error if directory passed in is not found', async () => {
    expect.assertions(1)
    try {
      await listSubdirectories(path.join(__dirname, 'NOTADIRECTORY'))
    } catch (e) {
