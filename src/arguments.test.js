...
describe('arguments', () => {
  ...
  test('if directory argument is not string, the message explains which argument is incorrect', async () => {
    expect.assertions(1)
    await expect(listSubdirectories(1)).rejects.toThrow('directory must be a string')
  })
  ...
  test('if options argument is not an object, the message explains which argument is incorrect', async () => {
    expect.assertions(1)
    await expect(listSubdirectories('.', 1)).rejects.toThrow('options must be an object')
  })

  test('if filter option is not a string, a TypeError is thrown', async () => {
    expect.assertions(1)
    await expect(listSubdirectories('.', { filter: 1 })).rejects.toThrow(TypeError)
  })

  test('if filter option is not a string, the message explains which argument is incorrect', async () => {
    expect.assertions(1)
    await expect(listSubdirectories('.', { filter: 1 })).rejects.toThrow('filter must be a string')
  })

  test('if levels option is not a number, a TypeError is thrown', async () => {
    expect.assertions(1)
    await expect(listSubdirectories('.', { levels: 'fdas' })).rejects.toThrow(TypeError)
  })

  test('if levels option is not a number, the message explains which argument is incorrect', async () => {
    expect.assertions(1)
    await expect(listSubdirectories('.', { levels: 'fdas' })).rejects.toThrow(
  ...
  test('if recursive option is not a boolean, a TypeError is thrown', async () => {
    expect.assertions(1)
    await expect(listSubdirectories('.', { recursive: 'fdas' })).rejects.toThrow(TypeError)
  })

  test('if recursive option is not a boolean, the message explains which argument is incorrect', async () => {
    expect.assertions(1)
    await expect(listSubdirectories('.', { recursive: 'fdas' })).rejects.toThrow(
  ...
  test('if levels option is zero, a RangeError is thrown', async () => {
    expect.assertions(1)
    await expect(listSubdirectories('.', { levels: 0 })).rejects.toThrow(RangeError)
  })

  test('if levels option is zero, the message explains why the argument is out of range', async () => {
    expect.assertions(1)
    await expect(listSubdirectories('.', { levels: 0 })).rejects.toThrow(
  ...
  test('if levels option is negative, a RangeError is thrown', async () => {
    expect.assertions(1)
    await expect(listSubdirectories('.', { levels: -5 })).rejects.toThrow(RangeError)
  })

  test('if levels option is a floating point, a RangeError is thrown', async () => {
    expect.assertions(1)
    await expect(listSubdirectories('.', { levels: 3.14 })).rejects.toThrow(RangeError)
  })

  test('if levels option and recursive option are specified, a RangeError is thrown', async () => {
    expect.assertions(1)
    await expect(listSubdirectories('.', { levels: 3, recursive: true })).rejects.toThrow(
      RangeError,
    )
  })
